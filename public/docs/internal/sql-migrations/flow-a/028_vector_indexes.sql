-- IntegrateWise OS: pgvector HNSW Indexes for Semantic Search
...
-- Drop legacy IVFFlat indexes (HNSW supersedes them with better recall and no
-- need for ANALYZE after bulk inserts)
DROP INDEX IF EXISTS brainstorm_sessions_embedding_ivfflat;
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'Created HNSW vector index on documents table';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    -- Drop legacy IVFFlat index
    DROP INDEX IF EXISTS interactions_embedding_ivfflat;
    ...
    RAISE NOTICE 'Created HNSW vector index on interactions table';
  END IF;
END $$;
