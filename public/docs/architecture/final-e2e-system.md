...
**Scope:** Multi-tenant continuity bridge, 26-service Cloudflare-native mesh, MCP-first ingress, governance-gated AI Twin, **100% swappable infrastructure layers**
...
## 1.1 The Swappable Dimensions (The 7× Agnostic Moat)

The platform is **deliberately swappable at every infrastructure layer**. The only fixed surface is the **capability contract** (Identity, Discovery, Projections, Capabilities, Events). Everything underneath is managed by the **Integration Manager** and can be swapped without consumer code changes.

| Dimension                    | Default                                                         | Swappable To                                                                                  | Managed By          |
| ---------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------- |
| **Frontend**                 | `apps/workbench` (Vite + React + shadcn/ui, Cloudflare Pages)   | Next.js, Electron, Lovable, Replit, React Native, Browser Extension, iOS, Android             | Integration Manager |
| **Auth**                     | Gateway-issued JWT + API Keys                                   | Clerk, Stack Auth, Descope, Auth0, Custom OAuth                                               | Integration Manager |
| **Relational Store (Spine)** | PostgreSQL via **HyperDrive** (Cloudflare's connection pooling) | Any PostgreSQL (Neon, Supabase, AWS RDS, CockroachDB, PlanetScale), any SQL-compatible engine | Integration Manager |
| **Cache / KV**               | Cloudflare KV                                                   | Redis, Memcached, Upstash, DynamoDB                                                           | Integration Manager |
| **Queue**                    | Cloudflare Queues                                               | SQS, RabbitMQ, Kafka, BullMQ                                                                  | Integration Manager |
| **AI / Model**               | gpt-5-mini via **OpenRouter**                                   | Claude, Grok, Gemini, Ollama, local LLM, any OpenAI-compatible endpoint                       | Integration Manager |
| **Deployment**               | Cloudflare Workers                                              | Vercel, AWS Lambda, Fly.io, Kubernetes, self-hosted                                           | Integration Manager |

**Key principle:** The Integration Manager is the **capability adapter** that sits between the capability contract and the provider fabric. It makes swappability real by translating the fixed capability contract into provider-specific calls. Consumers never see the provider.

---

## 1.2 The Fixed Surface (What Never Changes)

| Layer                   | Fixed                                                                                       | Why                                                          |
| ----------------------- | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| **Capability Contract** | Identity, Discovery, Projections, Capabilities, Events                                      | This is the Bridge — the public API that all consumers speak |
| **Governance Gate**     | 0.70 / 0.85 confidence thresholds, HITL, audit                                              | Safety and trust are invariant                               |
| **Tenant Isolation**    | `WHERE tenant_id = ?` on every query, 403 fail-loud                                         | Security is non-negotiable                                   |
| **MCP Protocol**        | `mcp.integratewise.ai/v1/mcp` as the primary ingress                                        | AI-first is the product thesis                               |
| **The Loop**            | Observe → Understand → Propose → Govern → Execute → Writeback → Remember → Organize → Learn | The operational lifecycle is the product                     |

---

## 2. The Four Layers (Conceptual Law)

These layers are **conceptual** — they describe trust boundaries, not deployment units. The physical 0–16 topology is in §8.
...
│   ├── Context window optimization (Context Assembly, §8.7)
...
| 8   | **Shared Memory**           | `continuity`, `triage`                                             | D1 `memory` (active/staging/archived), KV memory decay           |
...
- **Memory scopes (FINAL):** `Personal` (human), `Work` (task/project), `Organization` (org-wide), `AI` (twin/audit). Audit is cross-cutting Compliance, not a memory scope.
...
**Resolution of §14.8:** DECISION 30 (Grok) is **superseded** by §20.2 FINAL. Twin default = gpt-5-mini via OpenRouter. Grok is available as a config option, not the default.
...
**No Supabase.** No Vercel. No AWS backend. No Neon PostgreSQL. DECISION 22 is complete in the template.
...
| 4   | HITL Durable Object using `idFromName("global")`           | Use `idFromName(tenant_id + user_id)`                         | `govern`                  |
...
| §14.5 Layer model (4-layer vs 10-layer vs L0-L3)  | **4-layer = conceptual law** (§2); **0-16 = physical topology** (§8); 10-layer and L0-L3 **retired**.                                                 | **Locked** |
...
│   ├── tier-3-runtime/       # hermes, triage, workflow, iw-agent-runtime, mcp-connector
...
_This is the canonical end-to-end system specification for the IntegrateWise Continuity Bridge v1.0.0-FINAL. All prior documents, diagrams, and decisions are superseded by this version where they conflict. Source-of-context conflicts are flagged in §24 and resolved in §20.2._
