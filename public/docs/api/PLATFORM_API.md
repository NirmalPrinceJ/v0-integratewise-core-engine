...
**Get credentials:** Sign in via OAuth (Google/GitHub) at `/api/v1/auth/descope?provider=google`
...
| `POST` | `/api/v1/workspace/connectors/nango-session` | Create sync session | `{"connector_id":"salesforce"}` | `{"session_id":"...","status":"active"}` |
| `POST` | `/api/v1/workspace/connectors/:id/disconnect` | Disconnect connector | - | `{"success":true}` |
| `POST` | `/api/v1/workspace/register-connector` | Register new connector | `{"provider":"salesforce","flowType":"A"}` | `{"success":true,"connector_id":"..."}` |
...
1. **Get credentials** — Sign in via OAuth at `/api/v1/auth/descope?provider=google`
